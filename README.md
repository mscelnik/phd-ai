...
| -- | -- | -- | -- |
| GPT-5.1-Codex-Mini | 1 | gpt-5.1-codex-mini-r1 | **Comprehensive and verifiable rewrite**. Clean Python package under `src/sootsim/` with gas-phase integration, Monte Carlo population-balance, CLI, YAML-backed configuration, refs/data folders, Sphinx docs, CI, and pre-commit. Tests cover each key subsystem plus integration use. Simulation outputs remain outside the repo and docs list the web sources (Wikipedia soot/acetylene). |
...
1. Run `make setup` to build the `.venv` defined in the Makefile and install dev dependencies.
2. Launch the solver via `python -m sootsim --config data/configs/acetylene_run.yaml` (or use `run_simulation.py`).
3. CSV outputs appear under `outputs/<run name>/` and can be visualized with Excel or any plotting tool. The repo documents the same sources in `docs/` and `refs/` for transparency.

Check `docs/usage.rst` for more detailed instructions or `Makefile` targets for lint/test/format steps.
